...
const HeadlineCards = () => {
  ...
  return (
    <div className="max-w-full mx-auto px-4 py-6 grid gap-6 xs:grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
      {news.map((article, index) => (
        <div
          key={index}
          className="bg-black shadow-md border border-[#00df9a] rounded-lg mb-5"
        >
          <a href={article.url} target="_blank" rel="noopener noreferrer">
            <img
              className="rounded-t-lg w-full h-48 object-cover" // Ensure image covers space responsively
          ...
          <div className="p-5">
            <a href={article.url} target="_blank" rel="noopener noreferrer">
              <h5 className="text-[#00df9a] font-bold text-xl tracking-tight mb-2">
                {article.title}
              </h5>
            </a>
            <p className="font-normal text-white mb-3 text-sm sm:text-base">
